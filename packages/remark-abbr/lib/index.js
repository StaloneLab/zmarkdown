...
function splitTextByAbbr (textNode, abbrDefinitions, seenAbbreviations, opts) {
  ...
  const matches = uniqueAbbreviations
    .map(abbr => ([abbr, textNode.value.indexOf(abbr.identifier)]))
...
export default function plugin (options) {
  ...
  // Inject handlers
  add('micromarkExtensions', abbr)
  add('fromMarkdownExtensions', abbrFromMarkdown(opts))
  add('toMarkdownExtensions', abbrToMarkdown())
}
